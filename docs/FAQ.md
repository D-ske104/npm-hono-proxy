...
### Q. このプロキシサーバー自体がサプライチェーン攻撃でハックされていない保証は？

**A.** 依存関係を極小化し、クリーンな構成にしています。

- 使用している本番依存関係は `hono`、`@hono/node-server`、`semver` のみです（`package.json` の `dependencies` を参照）。
- `semver`, `hono`, `@hono/node-server` は依存関係を持たない単一パッケージです。
- 開発用依存関係（`devDependencies`）はビルドやテスト専用であり、ランタイムには含まれません。
...
# 出力例
npm-hono-proxy@0.3.0
├── @hono/node-server@1.19.6
├── hono@4.10.7
└── semver@7.7.3
```

このように、ランタイムに取り込まれる依存の範囲を厳しく制限することで、サプライチェーン攻撃のリスクを低減しています。必要に応じて `npm ls --omit=dev` で依存ツリーを確認できます。
