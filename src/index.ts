...
console.info(`    npm --registry http://localhost:${PORT} install <package>`)
console.info('')
console.info('  現在のプロジェクトのみレジストリを設定してインストール')
console.info(`    npm config set registry http://localhost:${PORT}`)
console.info('    npm install <package>')
console.info('')
console.info('  動作確認 (curl): 上流からのパッケージメタデータ取得')
console.info(`    curl -s http://localhost:${PORT}/<package> | head`)
console.info('')
console.info('オプション:')
console.info(`  --port <number>               デフォルト 4873`)
console.info(`  --upstream <url>              デフォルト https://registry.npmjs.org`)
console.info(`  --quarantine-minutes <num>    デフォルト 30240`)
console.info(`  --log-format <text|ndjson>    デフォルト text`)
console.info('----------------------------------------')
